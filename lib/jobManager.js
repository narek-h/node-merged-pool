...
var JobManager = module.exports = function JobManager(options){
 ...
 //public members
    ...
    var blockHasher = algos['blockhash'].hash(options.coin);
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, hashMix, coin){
        ...
        blockHashInvalid = util.reverseBuffer(blockHasher(headerBuffer, job.rpcData.version, job.rpcData.previousblockhash, merkleRoot, parseInt(nTime, 16), parseInt(job.rpcData.bits, 16), job.rpcData.height, hashMix, nonce)).toString('hex');
        //Check if share is a block candidate (matched network difficulty)
        if(job.target.ge(headerBigNum)){
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
            blockHash = util.reverseBuffer(blockHasher(headerBuffer, job.rpcData.version, job.rpcData.previousblockhash, merkleRoot, parseInt(nTime, 16), parseInt(job.rpcData.bits, 16), job.rpcData.height, hashMix, nonce)).toString('hex');
        }
        else {
