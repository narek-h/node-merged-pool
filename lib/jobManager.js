...
var JobManager = module.exports = function JobManager(options){
 ...
 //public members
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, hashMix){
...
//        var headerHash = hashDigest(headerBuffer, nTimeInt);
        var headerHash = hashDigest(headerBuffer, nonce, this.currentJob.rpcData.height);
        ...
        var shareDiff = 0.00024;  //diff1 / headerBigNum.toNumber() * shareMultiplier;
        ...
        blockHashInvalid = blockHasher(headerBuffer, nonce, this.currentJob.rpcData.height).toString('hex');
        //Check if share is a block candidate (matched network difficulty)
        if (true) { //TODO if(job.target.ge(headerBigNum)){
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
            blockHash = blockHasher(headerBuffer, nonce, this.currentJob.rpcData.height).toString('hex');
