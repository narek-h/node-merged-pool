 ...
 //public members
    ...
    var blockHasher = (function () {
        switch (options.coin.algorithm) {
            ...
            case 'nrghash':
                return function(d) {
                    return util.reverseBuffer(algos['blockhash'].hash(options.coin).apply(this, arguments));
                }
            case 'scrypt-n':
            case 'sha1':
                return function (d) {
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, hashMix, coin){
        var coinbaseBuffer = new Buffer(hexStringToByte(coin));
		...
		var headerHash = hashDigest(headerBuffer, job.rpcData.version, job.rpcData.previousblockhash, merkleRoot, parseInt(nTime, 16), parseInt(job.rpcData.bits, 16), job.rpcData.height, hashMix, parseInt(nonce, 16));
		...
		blockHashInvalid = blockHasher(headerBuffer, job.rpcData.version, job.rpcData.previousblockhash, merkleRoot, parseInt(nTime, 16), parseInt(job.rpcData.bits, 16), job.rpcData.height, hashMix, parseInt(nonce, 16)).toString('hex');

        //Check if share is a block candidate (matched network difficulty)
        if(job.target.ge(headerBigNum)){
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
			blockHash = blockHasher(headerBuffer, job.rpcData.version, job.rpcData.previousblockhash, merkleRoot, parseInt(nTime, 16), parseInt(job.rpcData.bits, 16), job.rpcData.height, hashMix, parseInt(nonce, 16)).toString('hex');
        }
        else {
