...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData){
    ...
    var not_superblock = true;
    if (rpcData.superblocks_enabled) {
        if (rpcData.superblock.length > 0) {
            not_superblock = false;
    ...
    if (not_superblock) { //TODO(Narek): Confirm if this is the expected payee. changed 'rpcData.payee->rpcData.backbone.payee'to match the block template
	var payeeReward = 0;

        if (rpcData.backbone.amount) {
            payeeReward = rpcData.backbone.amount;
        } else {
        ...
        var payeeScript = util.addressToScript(rpcData.backbone.payee);
        txOutputBuffers.push(Buffer.concat([
            util.packInt64LE(payeeReward),
            util.varIntBuffer(payeeScript.length),
            payeeScript
...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, auxMerkleTree){
    ...
    var txInSequence = 0; //TODO(Narek): Confirm. Was 0 (seems always) set to 1
